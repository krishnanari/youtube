...
def Api():
    api = "AIzaSyCZUrrrkjBrC3PHQX_HjOcZoH1wUVf_JOg"
...
def insert_videos(video_data):
    for data in video_data:
        ...
        if not result:
            ...
            val = (data['channel_Name'], data['channel_Id'], data['video_Id'], data['Title'], data['Tags'], data['Thumbnail'], data['Description'], data['Published_date'], data['Duration'], data['view'], data['likes'], data['dislikes'], data['comments'])
            cursor.execute(sql, val)
    mydb.commit()
